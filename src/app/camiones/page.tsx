...
import { 
  ...
  UserCheck, Hash, Phone, Calendar, Info, Filter, Edit3, Trash2
} from 'lucide-react'
...
import { CamionStatsModal } from '@/components/CamionStatsModal'
import { ChoferModal } from '@/components/ChoferModal' // 🚀 Importamos el modal de chofer
...
export default function FlotaPage() {
  ...
  const initialChoferFormState = {
    nombre: '', dni: '', telefono: '', 
    licencia: '', vto_licencia: '', estado: 'Disponible', foto_url: ''
  }

  const [formData, setFormData] = useState<any>(initialFormState)
  const [choferFormData, setChoferFormData] = useState<any>(initialChoferFormState)
  ...
  async function fetchData() {
    ...
    try {
      const [ca, ch, ga, vi] = await Promise.all([
        ...
        supabase.from('choferes').select('*').order('nombre', { ascending: true }), // Traemos todo del chofer
        supabase.from('gastos_camion').select('*').order('fecha', { ascending: false }),
        supabase.from('viajes').select('camion_id, km_recorridos, lts_gasoil, fecha') 
      ]);
  ...
  const handleSubmitGasto = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchData();
    } catch (err: any) { alert("Error al cargar gasto: " + err.message); } 
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeTab === 'camiones') {
      return camiones.filter(c => {
        const matchSearch = c.patente.toLowerCase().includes(term) || 
        ...
        if (filterEstado === 'ACTIVOS') {
          ...
          matchEstado = est.includes('inactivo') || est.includes('vendido') || est.includes('baja');
        }
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 pb-32 font-sans italic selection:bg-cyan-500/30 overflow-x-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-6 lg:px-10 pt-24 md:pt-32 space-y-8 relative z-10">
        <header className="flex flex-col xl:flex-row justify-between items-end gap-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
            <div className="relative group flex-1 xl:w-96">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder={activeTab === 'camiones' ? "BUSCAR PATENTE..." : "BUSCAR CHOFER..."} className="w-full bg-slate-950 border border-white/10 rounded-3xl py-5 pl-14 text-white font-bold outline-none focus:border-cyan-500/50 uppercase italic" />
            </div>
            <div className="flex bg-slate-900/80 p-1.5 rounded-[2.5rem] border border-white/5 backdrop-blur-md shadow-2xl">
              <TabBtn active={activeTab === 'camiones'} onClick={() => {setActiveTab('camiones'); setFilterEstado('ACTIVOS')}} label="Camiones" />
              <TabBtn active={activeTab === 'choferes'} onClick={() => {setActiveTab('choferes'); setFilterEstado('ACTIVOS')}} label="Choferes" />
            </div>
            <button 
              onClick={() => { 
                setEditingId(null); 
        ...
        <div className="flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-2">
            ...
            <StatusFilterBtn active={filterEstado === 'NO_DISPONIBLES'} onClick={() => setFilterEstado('NO_DISPONIBLES')} label={activeTab === 'camiones' ? "En Taller" : "Franco / Lic"} color="text-amber-400" border="border-amber-500/30" bg="bg-amber-500/10" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredData.map((c) => (
                  <CamionCard 
                    ...
                    onShowStats={() => { setSelectedCamion(c); setIsStatsModalOpen(true); }} 
                  />
                ))}
              </div>
            ) : (
            ...
            filteredData.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {filteredData.map(ch => {
                   ...
                   let opacityClass = (estado === 'Inactivo' || estado === 'Inactivo / Desvinculado') ? 'opacity-50 grayscale' : '';
                   let statusColor = estado === 'En Viaje' ? 'text-cyan-400 border-cyan-400/20' : 
                                     (estado === 'Franco' || estado === 'Franco / Licencia') ? 'text-amber-400 border-amber-400/20' : 
                                     (estado === 'Inactivo' || estado === 'Inactivo / Desvinculado') ? 'text-rose-400 border-rose-400/20' : 
                                     'text-emerald-400 border-emerald-400/20';

                   return (
                     <div key={ch.id} className={`p-8 bg-slate-950/40 rounded-[2.5rem] border border-white/5 backdrop-blur-xl group hover:border-indigo-500/30 transition-all shadow-2xl relative overflow-hidden flex flex-col justify-between ${opacityClass}`}>
                       <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:scale-125 transition-transform duration-700 pointer-events-none">
                         {ch.foto_url ? <img src={ch.foto_url} className="w-32 h-32 rounded-full object-cover" /> : <UserCircle size={120} />}
                       </div>
                       
                       <div>
                         <div className="flex justify-between items-start mb-4">
                           <h3 className="text-2xl font-black uppercase italic tracking-tighter text-white leading-none max-w-[70%]">{ch.nombre}</h3>
                           {ch.foto_url && <img src={ch.foto_url} className="w-12 h-12 rounded-2xl object-cover border border-white/10" />}
                         </div>
                         <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border ${statusColor}`}>
                            {estado}
                         </span>
                       </div>
                       ...
                       <div className="space-y-3 pt-6 mt-6 border-t border-white/5 relative z-10">
                          ...
                          <ChoferInfoRow icon={ShieldAlert} label="Licencia" val={ch.vto_licencia ? new Date(ch.vto_licencia).toLocaleDateString('es-AR',{timeZone:'UTC'}) : '---'} />
                       </div>
                       ...
                       <div className="mt-6 pt-6 border-t border-white/5 flex gap-2 relative z-10">
                         <button 
                           onClick={() => {
                             setEditingId(ch.id);
                             setChoferFormData({ ...initialChoferFormState, ...ch });
                             setIsChoferModalOpen(true);
                           }} 
      ...
      <CamionStatsModal isOpen={isStatsModalOpen} onClose={() => setIsStatsModalOpen(false)} camion={selectedCamion} viajes={viajes} gastos={gastos} />
...
function StatusFilterBtn({ active, onClick, label, color, border, bg }: any) {
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${active ? `${bg} ${border} ${color}` : 'border-transparent text-slate-500 hover:bg-white/5'}`}>
      {label}
    </button>
  )
}
function EmptyState({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-white/5 rounded-[3rem] bg-slate-900/20 backdrop-blur-sm">
      ...
      <h3 className="text-lg font-black text-slate-500 uppercase tracking-widest">Sin resultados</h3>
      <p className="text-[10px] text-slate-600 uppercase tracking-widest mt-2">{message}</p>
    </div>
  )
}