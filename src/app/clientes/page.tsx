...
import { 
  ...
  Inbox, TrendingUp, GripVertical, Trash2, Mail, DownloadCloud, AlertTriangle, X, Edit3, Menu
} from 'lucide-react'
...
export default function ClientesPage() {
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-100 overflow-hidden font-sans italic selection:bg-sky-500/30">
      ...
      <main className="flex-1 overflow-y-auto relative z-10">
        ...
        {selected ? (
          <div className="max-w-7xl mx-auto p-6 lg:p-10 space-y-8 lg:space-y-12 relative animate-in fade-in duration-500 pb-32">
            ...
            {/* HEADER CLIENTE - Adaptado a Grid para Responsive */}
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-8">
              <div className="space-y-4">
                <p className="text-[10px] font-black text-sky-500 uppercase tracking-[0.4em] font-sans not-italic italic">Gestión / {selected.cuit}</p>
                <div className="flex flex-wrap items-center gap-4 lg:gap-6">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setIsEditModalOpen(true)} className="p-2 bg-white/5 rounded-xl hover:bg-sky-500/20 hover:text-sky-500 border border-white/5 transition-all"><Edit3 size={18} /></button>
                    <button onClick={handleDeleteCliente} className="p-2 bg-white/5 rounded-xl hover:bg-rose-500/20 hover:text-rose-500 border border-white/5 transition-all"><Trash2 size={18} /></button>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mt-6">
                  <div className="px-6 py-3 bg-emerald-500/10 rounded-full border border-emerald-500/20 shadow-lg">
                    <span className="text-[10px] font-black text-emerald-500/50 uppercase mr-3 tracking-widest italic text-emerald-500">Saldo Pendiente:</span>
            ...
            >
              <h2 className="text-[11px] font-black uppercase tracking-[0.4em] opacity-40 flex items-center gap-3 italic"><Inbox size={18} /> Libro Mayor</h2>
              <div className="grid grid-cols-1 gap-3">
                {gestion.maestro.length > 0 ? gestion.maestro.map((m: any) => (
                  <div key={m.id} draggable onDragStart={(e) => onDragStart(e, m.id)} onDragEnd={onDragEnd} className="bg-slate-900/40 p-5 lg:p-6 rounded-[1.5rem] lg:rounded-[2.5rem] border border-white/5 flex justify-between items-center group cursor-grab active:cursor-grabbing hover:border-sky-500/30 transition-all">
                    <div className="flex items-center gap-4 lg:gap-8 text-white">
                      ...
                      <div>
                        <p className="text-[9px] font-black text-slate-500 uppercase italic leading-none mb-1">Carga Reciente</p>
                        <p className="text-lg lg:text-xl font-black italic tracking-tighter text-white font-sans leading-none">$ {Number(m.debe || m.monto).toLocaleString('es-AR')}</p>
                      </div>
                    </div>
                  </div>
                )) : (
                  <p className="text-[10px] text-slate-600 uppercase tracking-widest text-center py-4">Sin facturas para clasificar</p>
                )}
              </div>
            </section>

            {/* CAJAS DE BALANCE - Responsive: 1 col en móvil, 2 en desktop */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
              <div onDragOver={(e) => {e.preventDefault(); setIsOverBox('por_cobrar')}} onDragLeave={() => setIsOverBox(null)} onDrop={(e) => onDrop(e, 'por_cobrar')} className={`border p-6 lg:p-10 rounded-[2.5rem] lg:rounded-[4rem] min-h-[300px] lg:min-h-[500px] flex flex-col transition-all duration-300 ${isOverBox === 'por_cobrar' ? 'bg-emerald-500/10 border-emerald-500 scale-[1.02]' : 'bg-emerald-500/[0.02] border-emerald-500/10'}`}>
                ...
                <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                  {gestion.porCobrar.map((m: any) => (
                    <div key={m.id} draggable onDragStart={(e) => onDragStart(e, m.id)} onDragEnd={onDragEnd} className="bg-slate-950 p-5 lg:p-6 rounded-[1.5rem] lg:rounded-[2rem] border border-emerald-500/10 flex justify-between items-center group cursor-grab hover:border-emerald-500/40">
                      <p className="text-lg lg:text-xl font-black italic tracking-tighter font-sans leading-none text-white">${Number(m.debe || m.monto).toLocaleString()}</p>
              ...
              <div onDragOver={(e) => {e.preventDefault(); setIsOverBox('cobrado')}} onDragLeave={() => setIsOverBox(null)} onDrop={(e) => onDrop(e, 'cobrado')} className={`border p-6 lg:p-10 rounded-[2.5rem] lg:rounded-[4rem] min-h-[300px] lg:min-h-[500px] flex flex-col transition-all duration-300 ${isOverBox === 'cobrado' ? 'bg-rose-500/10 border-rose-500 scale-[1.02]' : 'bg-rose-500/[0.02] border-rose-500/10'}`}>
                ...
                <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                  {gestion.cobrados.map((m: any) => (
                    <div key={m.id} draggable onDragStart={(e) => onDragStart(e, m.id)} onDragEnd={onDragEnd} className="bg-slate-950 p-5 lg:p-6 rounded-[1.5rem] lg:rounded-[2rem] border border-rose-500/10 flex justify-between items-center opacity-40 hover:opacity-100 transition-all cursor-grab hover:border-rose-500/40">
                      <p className="text-lg lg:text-xl font-black italic tracking-tighter font-sans leading-none text-white">${Number(m.debe || m.monto).toLocaleString()}</p>
                      <GripVertical size={16} className="text-slate-800" />
                    </div>
                  ))}
            ...
            {/* TACHO DE BASURA - Adaptado para móvil (más pequeño) */}
            <div onDragOver={(e) => { e.preventDefault(); setIsOverBox('delete') }} onDrop={(e) => onDrop(e, 'delete')} className={`fixed bottom-6 lg:bottom-10 left-1/2 -translate-x-1/2 z-[100] transition-all duration-500 flex flex-col items-center gap-3 ${isDragging ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20 pointer-events-none'}`}>
              <div className={`p-6 lg:p-8 rounded-full border-2 border-dashed transition-all ${isOverBox === 'delete' ? 'bg-rose-600 border-white scale-110 text-white shadow-[0_0_50px_rgba(225,29,72,0.5)]' : 'bg-rose-900/20 border-rose-500/50 text-rose-500'}`}>
                <Trash2 size={32} className={isOverBox === 'delete' ? 'animate-bounce' : ''} />
              </div>
      ...
      {/* MODAL ALTA CLIENTE - Responsive (w-full en móvil) */}
      {isClientModalOpen && (
      ...
      {/* MODAL EDICIÓN CLIENTE - Responsive */}
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 lg:p-6 bg-black/90 backdrop-blur-md">
          <div className="bg-[#0f172a] border border-sky-500/30 p-8 lg:p-12 rounded-[2.5rem] lg:rounded-[3.5rem] w-full max-w-lg relative italic shadow-2xl">
            ...
            <form onSubmit={handleUpdateCliente} className="space-y-4">
              ...
              <button disabled={isSaving} className="w-full py-5 lg:py-6 bg-emerald-600 text-white font-black rounded-[1.5rem] lg:rounded-[2rem] uppercase text-[11px] tracking-[0.2em] mt-6 shadow-xl active:scale-95 transition-all">
