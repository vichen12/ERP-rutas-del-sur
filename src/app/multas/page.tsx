...
import { CostosFijosSection } from '@/components/CostosFijosSection'
import { MultasSection } from '@/components/MultasSection'
import { PuntoEquilibrioSection } from '@/components/PuntoEquilibrioSection'
import { CostoModal } from '@/components/CostoModal'
import { MultaModal } from '@/components/MultaModal'
import { registrarMovimiento, registrarPagoMulta } from '@/lib/cajaService'
...
export default function CostosMulatasPage() {
  ...
  async function handleSaveCosto(data: any) {
    ...
    try {
      if (editingCosto) {
        ...
        const { data: nuevo, error } = await supabase.from('costos_fijos').insert([data]).select().single()
        if (error) throw error

        // ── IMPACTO EN CAJA: registrar egreso del costo fijo ─────────────────
        const monto = Number(data.monto || 0)
        if (monto > 0) {
          await registrarMovimiento({
            tipo:                 'egreso',
            categoria:            'costo_fijo',
            descripcion:          data.nombre || 'COSTO FIJO',
            monto:                data.es_anual ? monto / 12 : monto,
            tipo_cuenta:          'banco',
            referencia_origen_id: nuevo?.id,
            modulo_origen:        'costos_fijos',
          })
        }
        // ─────────────────────────────────────────────────────────────────────
      }
  ...
  async function handlePagarMulta(multa: any) {
    ...
    // ── IMPACTO EN CAJA: registrar egreso de la multa ─────────────────────────
    const monto = Number(multa.monto || 0)
    if (monto > 0) {
      const chofer = choferes.find(c => c.id === multa.chofer_id)
      const camion = camiones.find(c => c.id === multa.camion_id)
      await registrarPagoMulta({
        monto,
        descripcion:  multa.descripcion || multa.motivo || 'MULTA',
        camion_id:    multa.camion_id   || null,
        chofer_id:    multa.chofer_id   || null,
        multaId:      multa.id,
