    ...
    <div className="space-y-8 font-sans italic selection:bg-cyan-500/30">
      ...
      <header className="flex flex-col xl:flex-row justify-between items-stretch gap-8">
        <div className="flex flex-col justify-between space-y-8 w-full xl:w-auto">
          ...
          <div className="bg-[#141c28]/40 border border-white/5 p-6 rounded-[2.5rem] backdrop-blur-xl flex items-center gap-6 group hover:border-amber-500/20 transition-all duration-500 shadow-2xl">
        ...
        <div className="flex-1 max-w-full xl:max-w-xl bg-[#1a2537]/40 border border-white/10 rounded-[2.8rem] p-7 backdrop-blur-md flex flex-col h-[320px] shadow-2xl group relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-6 relative z-10">
            ...
            <span className="bg-[#141c28] px-3 py-1.5 rounded-full text-[8px] font-black text-slate-500 uppercase border border-white/5">{recordatorios.length} CARGAS</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 mb-6 relative z-10">
             {recordatorios.length === 0 ? (
               ...
               <div key={r.id} className="flex justify-between items-center group/item bg-[#141c28]/50 p-4 rounded-2xl border border-white/5 hover:border-cyan-500/30 transition-all">
                 ...
                 <button onClick={() => handleDeleteRecordatorio(r.id)} className="text-slate-600 hover:text-rose-500 p-2 bg-[#141c28]/40 rounded-xl opacity-0 group-hover/item:opacity-100 transition-all"><Trash2 size={14} /></button>
               </div>
             ))}
          </div>
          
          <form onSubmit={handleAddRecordatorio} className="relative z-10">
            <input value={nuevoRecordatorio} onChange={e => setNuevoRecordatorio(e.target.value)} placeholder="PROGRAMAR NUEVA RUTA / DESPACHO..." className="w-full bg-[#141c28]/80 border border-white/10 rounded-2xl py-4.5 pl-6 pr-14 text-[10px] text-white font-black uppercase outline-none focus:border-cyan-500 placeholder:text-slate-800" />
      ...
      <div className="flex flex-col xl:flex-row items-stretch xl:items-center gap-4 bg-[#141c28]/40 p-2 rounded-[2.2rem] border border-white/5">
        ...
        <div className="flex bg-[#1a2537] p-1.5 rounded-3xl border border-white/5 shadow-inner">
           <button 
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-6 py-4 bg-[#1a2537] border border-white/10 rounded-[1.8rem] text-slate-400 hover:text-white font-black uppercase text-[9px] tracking-widest transition-all">
          <Navigation size={14} /> GPS / Traccar
        </button>

        <button onClick={() => setShowAllTime(!showAllTime)} className={`flex items-center justify-center gap-3 px-8 py-4 rounded-[1.8rem] text-[9px] font-black uppercase tracking-[0.2em] transition-all border ${showAllTime ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg' : 'bg-[#1a2537] border-white/10 text-slate-500 hover:text-white'}`}>
      ...
      <div className="flex flex-col xl:flex-row gap-4 items-center bg-[#141c28]/20 p-2 rounded-[3rem] border border-white/5">
          <div className="flex bg-[#141c28] p-1.5 rounded-[2.5rem] w-full xl:w-auto overflow-x-auto no-scrollbar shadow-inner">
          ...
          <div className="relative flex-1 w-full group">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="BUSCAR CLIENTE, PATENTE O DESTINO..." className="w-full bg-[#141c28]/80 border border-white/10 rounded-[2.2rem] py-5 pl-14 pr-8 text-white font-black outline-none focus:border-cyan-500/40 uppercase tracking-[0.1em] placeholder:text-slate-800 transition-all text-[10px]" />
          </div>
...
function StatCard({ title, value, icon: Icon, color, highlight }: any) {
  return (
    <div className={`bg-[#1a2537]/40 p-7 md:p-8 rounded-[2.2rem] border transition-all duration-500 group relative overflow-hidden ${highlight ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/5'}`}>
...
function SubStat({ label, value, icon: Icon, color }: any) {
  return (
    <div className="bg-[#1a2537]/20 p-5 rounded-[1.8rem] border border-white/5 flex items-center gap-5 group hover:bg-[#1a2537]/40 transition-all h-full">
      <div className={`p-3 rounded-xl bg-[#141c28] ${color} border border-white/5 shadow-inner shrink-0`}>
        <Icon size={20} />
      </div>
      <div>
