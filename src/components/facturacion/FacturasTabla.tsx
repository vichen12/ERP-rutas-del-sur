...
export function FacturasTabla({ facturas, loading, onDelete }: any) {
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left min-w-[900px]">
            <thead className="bg-white/[0.02] text-[9px] font-black text-slate-600 uppercase tracking-[0.3em] border-b border-white/5">
              <tr><th className="p-6 pl-8">Fecha</th><th className="p-6">Tipo</th><th className="p-6">Comprobante</th><th className="p-6">Cliente</th><th className="p-6">CAE</th><th className="p-6">Vto</th><th className="p-6 text-right">Total</th><th className="p-6 text-center">Estado</th><th className="p-6 pr-8 text-center">Acción</th></tr>
            </thead>
            <tbody className="divide-y divide-white/[0.03]">
              {facturas.length === 0 && <tr><td colSpan={9} className="py-20 text-center"><div className="flex flex-col items-center gap-3 opacity-30"><FileText size={40} className="text-slate-600" /><p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Sin facturas</p></div></td></tr>}
              {facturas.map((f: any) => {
                const l = TIPO_MAP[Number(f.tipo_comprobante)] || String(f.tipo_comprobante) || 'B'
                const puedeEliminar = f.estado === 'manual' || f.estado === 'pendiente' || f.estado === 'error'
                return (
                  <tr key={f.id} className="hover:bg-white/[0.02] transition-all">
                    ...
                    <td className="p-6"><p className="text-sm font-black text-white tabular-nums">{String(f.punto_venta).padStart(4,'0')}-{String(f.numero_comprobante||0).padStart(8,'0')}</p></td>
                    ...
                    <td className="p-6 text-center"><span className={'px-3 py-1.5 rounded-xl text-[8px] font-black uppercase border '+ec(f.estado)}>{f.estado}</span></td>
            ...
            return (
              <div key={f.id} className="p-6 space-y-3">
                <div className="flex justify-between items-center">
                  <span className={'px-3 py-1 rounded-xl text-[9px] font-black uppercase border '+(TC[l]||TC.B)}>FACT {l}</span>
                  <div className="flex items-center gap-2">
                    <span className={'px-3 py-1 rounded-xl text-[9px] font-black uppercase border '+ec(f.estado)}>{f.estado}</span>
                    {puedeEliminar && onDelete && (
                      <button onClick={() => onDelete(f)} className="p-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500 hover:text-white transition-all">
                        <Trash2 size={14} />
                      </button>
                    )}
                  </div>
                </div>
                <p className="text-base font-black text-white uppercase">{f.clientes?.razon_social||'CONS. FINAL'}</p>
