...
export function CargaCombustibleModal({ isOpen, onClose, onSave, camiones, choferes, precioGasoilReferencia }: CargaCombustibleModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto bg-[#141c28]/80 backdrop-blur-sm p-4 font-sans italic" onClick={onClose}>
      <div className="bg-[#141c28] border border-amber-500/20 rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl relative animate-in fade-in zoom-in-95 duration-300 my-auto max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={formData.fecha} onChange={e => setFormData({...formData, fecha: e.target.value})} required className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white outline-none" />
            </div>
            <div>
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Nº Remito (Opcional)</label>
              <input type="text" value={formData.remito_nro} onChange={e => setFormData({...formData, remito_nro: e.target.value})} placeholder="Ej: 0001-1234" className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white outline-none placeholder:text-slate-700" />
            </div>
          </div>

          {/* SELECTOR DE TIPO DE RESPONSABLE */}
          <div className="flex bg-[#1a2537] p-1 rounded-xl border border-white/10">
          ...
          <div>
            ...
            {tipoResponsable === 'chofer' ? (
              <select value={formData.chofer_id} onChange={e => setFormData({...formData, chofer_id: e.target.value})} required className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white outline-none">
                <option value="">Seleccione chofer...</option>
                {choferes.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
              </select>
            ) : (
              <input type="text" value={formData.responsable_externo} onChange={e => setFormData({...formData, responsable_externo: e.target.value})} required placeholder="Ej: Encargado Depósito / Camioneta" className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white outline-none placeholder:text-slate-700" />
            )}
          </div>

          <div>
            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Unidad / Camión (Opcional)</label>
            <select value={formData.camion_id} onChange={e => setFormData({...formData, camion_id: e.target.value})} className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white outline-none">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.1" value={formData.litros} onChange={e => setFormData({...formData, litros: e.target.value})} required className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white outline-none font-bold text-lg tabular-nums" />
            </div>
            <div>
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Precio por Litro</label>
              <input type="number" step="0.1" value={formData.precio_litro} onChange={e => setFormData({...formData, precio_litro: e.target.value})} required className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-amber-500 outline-none font-bold text-lg tabular-nums" />
            </div>
          </div>

          <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between bg-[#141c28]/20 p-4 rounded-2xl">
          ...
          <div className="flex gap-3 mt-6">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-3 bg-[#243248] text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-slate-700 transition-all">Cancelar</button>
