...
export function PagarResumenModal({ isOpen, onClose, onConfirm, cargasSeleccionadas }: PagarResumenModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto bg-[#141c28]/80 backdrop-blur-sm p-4 font-sans italic" onClick={onClose}>
      <div className="bg-[#141c28] border border-emerald-500/20 rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl relative animate-in fade-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto my-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
           <div className="bg-emerald-950/20 border border-emerald-500/20 p-5 rounded-[2rem] space-y-4 shadow-inner">
             ...
             <div className="grid grid-cols-2 gap-4">
               <div>
                  ...
                  <input type="number" step="0.1" value={formData.ltsReales} onChange={e => setFormData({...formData, ltsReales: e.target.value})} required className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-3 text-white font-bold outline-none tabular-nums" />
               </div>
               <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Monto a Transferir ($)</label>
                  <input type="number" step="0.1" value={formData.montoReal} onChange={e => setFormData({...formData, montoReal: e.target.value})} required className="w-full bg-[#1a2537] border border-emerald-500/30 rounded-xl p-3 text-emerald-400 font-bold outline-none tabular-nums" />
               </div>
             </div>
           </div>
           ...
           <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" value={formData.fecha} onChange={e => setFormData({...formData, fecha: e.target.value})} required className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-4 text-white font-bold outline-none uppercase text-xs" />
              </div>
              <div>
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Origen del dinero</label>
                <select value={formData.metodo} onChange={e => setFormData({...formData, metodo: e.target.value})} className="w-full bg-[#1a2537] border border-white/10 rounded-xl p-4 text-white font-bold outline-none uppercase text-xs">
            ...
            <button type="button" onClick={onClose} className="flex-1 px-4 py-4 bg-[#243248] text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-slate-700 transition-all">Cancelar</button>
